"""
    ...
    representation of exmo exchanges market symbols.
"""

from crypto_summoner.symbols_interface.symbols_interface import SymbolsInterface
from crypto_summoner.symbols_interface.exmo_symbols_enum import ExmoSymbolsEnum
from crypto_summoner.symbols_interface.symbols_enum import SymbolsEnum


class ExmoSymbols(SymbolsInterface):
    """
    ExmoSymbols Class

    ExmoSymbols is an concrete implementation of
    SymbolsInterface class.

    It will for provide exmoExchangeWrapper
    access ExmoSymbolsEnum
    ...
    ...
    _symbols_enum = ExmoSymbolsEnum
    _symbols_enum_invert_dictionary = {v.value: k for k, v in ExmoSymbolsEnum.__members__.items()}
...
def test_exmo_symbols():
    print(ExmoSymbols.is_valid())
    s = ExmoSymbols.get_symbols_enum()
    for enum in list(s):
        print(enum.value)
    print(ExmoSymbols.get_exchange_symbol_by_standard_name('ETHBTC'))
