"""
    IExchangeFactory is an abstract class for initializing
     IExchangeWrapper concrete implementations.
...
class IExchangeFactory(ABC):
    ...
    def register_exchange(self, exchange_name, exchange_wrapper_class_reference):
        """
        Registers an IExchangeWrapper in this class
         _registered_exchange_wrappers dictionary.

        It registeres an IExchangeWrapper concrete implementation
         class by adding {'{exchange_name}':exchange_wrapper_class_reference}
         to initialized_exchange_wrappers dictionary.

        Args:
            exchange_name (str): Name of the exchange we want to initialize.

            exchange_wrapper_class_reference (IExchangeWrapper):
             IExchangeWrapper concrete implementation class reference.
    ...
    def registered_exchange_wrappers(self, registered_exchange_wrappers_dic):
        ...
        Args:
            registered_exchange_wrappers_dic (dic): Dictionary of names
             and classes which inherited from IExchangeWrapper class
             and used decorator to register in IExchangeFactory.
