"""
    GeneralExchangeFactory is a class inherited from IExchangeFactory
     for initializing IExchangeWrapper concrete implementations.

    Any Exchange SDK is wrapped in a class inheriting from
     IExchangeWrapper abstract class.
...
class GeneralExchangeFactory(IExchangeFactory):
    ...
    ExchangeFactoryGeneral is a class inherited from
     IExchangeFactory for initializing IExchangeWrapper
     concrete implementations.

    Any Exchange SDK is wrapped in a class inheriting
     from IExchangeWrapper abstract class.
    ...
    Attributes:
        _registered_exchange_wrappers (dic): Dictionary of
         names and classes which inherited from IExchangeWrapper
         class and registered.

        _initialized_exchange_wrappers (dic): Dictionary of
         names and classes which inherited from IExchangeWrapper
         class and initialized.

    Methods:
        create_exchange(self, exchange_name, credential_dic):
         Creates a concrete object of the requested exchange wrapper.
        register_exchange(self, exchange_name, exchange_wrapper_class_reference):
    ...
    def __init__(self):
        ...
        It initializes _registered_exchange_wrappers and
         _initialized_exchange_wrappers dictionaries.

        Attributes:
        _registered_exchange_wrappers (dic): Dictionary of names
         and classes which inherited from IExchangeWrapper class
         and registered.

        _initialized_exchange_wrappers (dic): Dictionary of names
         and classes which inherited from IExchangeWrapper class
         and initialized.
        ...
        logger.info(
            f"Registered Exchange Wrappers Classes:\n\
    ...
    async def create_exchange(self, exchange_name, credential_dic):
        ...
        try:
            ...
            logger.exception(
                f"Failed to initialize exchange wrapper by the name of {exchange_name}"
            )

    def register_exchange(self, exchange_name, exchange_wrapper_class_reference):
        """
        Registers an IExchangeWrapper in this class _registered_exchange_wrappers
         dictionary.

        It registeres an IExchangeWrapper concrete implementation class by adding
         {'{exchange_name}':exchange_wrapper_class_reference}
         to initialized_exchange_wrappers dictionary.

        Args:
            exchange_name (str): Name of the exchange we want to initialize.
            exchange_wrapper_class_reference (IExchangeWrapper): IExchangeWrapper
             concrete implementation class reference.
    ...
    def register_all_exchanges(self):
        """
        Registers all IExchangeWrapper concrete implementations
         which is in ExchangeWrapperRegistery.

        It registeres all IExchangeWrapper concrete implementation
         class by calling register_exchange method.
    ...
    async def create_all_exchanges(self):
        ...
        It is definde as async function because this code might be
         Network-bound due to its usage of create_exchange method.
    ...
    def get_exchange_wrapper(self, exchange_name):
        """
        Returns an initialized instance of the exchange wrapper class
         by the name of exchange_name.

        It an initialized instance of the exchange wrapper class
         by the name of exchange_name.

        If exchange_name does not exist it returns None.

        Args:
            exchange_name (str): Name of the exchange we want to retreive.

        Returns:
            If exchange_name found in _initialized_exchange_wrappers it
             returns IExchangeWrapper, otherwise None.

        """

        return self.initialized_exchange_wrappers.get(exchange_name, None)
    ...
    def registered_exchange_wrappers(self, registered_exchange_wrappers_dic):
        ...
        Args:
            registered_exchange_wrappers_dic (dic): Dictionary of names
             and classes which inherited from IExchangeWrapper class and
             used decorator to register in IExchangeFactory.
             e.g. {'Binance': BinanceExchangeWrapper}
    ...
    def initialized_exchange_wrappers(self, initialized_exchange_wrappers_dic):
        ...
        Args:
            initialized_exchange_wrappers_dic (dic): Dictionary of names
             and classes which inherited from IExchangeWrapper class
             and initialized using the create_exchange.
             e.g. {'Binance': BinanceExchangeWrapper}
...
async def test_general_exchange_factory():
    ...
    # bew = gef.get_exchange_wrapper("BinanceExchangeWrapper")
    # prices = await bew.get_prices()
    # print(prices)
    logger.info("Integration upto this point is OK.")
    logger.info("Feature will be added and commited.")
