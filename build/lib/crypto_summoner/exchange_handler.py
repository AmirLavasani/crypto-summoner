"""
    ExchangeHandler is an abstract class for wrapping exchanges SDKs.
...
class ExchangeHandler(ABC):
    ...
    def __init__(self, credential_dic):
        """
        Inits ExchangeHandler with exchange credential dic.
        Call this init in derived __init__ functions. 
        class derived(ExchangeHandler):
            def __init():
    ...
    def register_exchange_handler():
        ...
        raise NotImplementedError(f"{self.register.__name__} is an abstract method. Override it in subclasses.")

async def main():
    class TestExchangeHandler(ExchangeHandler):
        def __init__(self, credential_dic):
            ExchangeHandler.__init__(self, credential_dic)
    ...
    exhan = TestExchangeHandler({'API_KEY':'123'})
